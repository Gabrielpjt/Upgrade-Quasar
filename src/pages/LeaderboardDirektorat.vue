<template>
  <q-page>
    ...
    <LeaderboardBar :data="{title: 'Leaderboard by Direktorat', icon: 'domain'}"/>
...
<!--                  :hint="filter.result_info"-->
              ...
              <q-space />
              <div class="q-mt-sm">
                <span class="kmp-font-sm">{{filter.result_info}}</span>
<!--                <q-pagination-->
<!--                  v-model="current"-->
<!--                  :max="10"-->
<!--                  input-->
...
import { leaderboardDirektoratPointHistory } from 'src/lib/api';
  ...
  methods: {
    ...
    filterData() {
      if (this.filter.value !== '' && this.filter.value !== null) {
        ...
        this.filter.result_info = `Search for '${this.filter.value.toLowerCase()}', ${this.leaderboard.data.length} direktorat found in leaderboard`;
      } else {
        this.resetData();
      }
    },
