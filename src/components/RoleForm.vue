  ...
  <q-dialog ref="modal" minimized>
    <q-card>
      <q-card-section class="bg-primary text-white row">
        <div class="text-h6 col">Form {{currentAction}} Role</div>
...
import { roleCreate, roleUpdate, roleDelete, roleRestore } from '../lib/api';
  ...
  methods: {
    create() {
      ...
      if (this.$v.tempData.$invalid) {
        ...
        roleCreate({ name, display_name, description })
          .then(payload => {
            if (!payload.errors) {
    ...
    update() {
      ...
      if (this.$v.tempData.$invalid) {
        ...
        roleUpdate(this.tempData.id, { name, display_name, description })
          .then(payload => {
            if (!payload.errors) {
        ...
        .then(payload => {
          if (!payload.errors) {
            ...
            Notify.create({
              ...
              color: 'positive',
              icon: 'done',
              message: 'Data role berhasil dinon-aktifkan'
            });
          }
        ...
        .then(payload => {
          if (!payload.errors) {
            ...
            Notify.create({
              ...
              color: 'positive',
              icon: 'done',
              message: 'Data role berhasil diaktifkan kembali'
            });
          }
          this.isLoading = false;
