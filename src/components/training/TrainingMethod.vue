...
<template>
            ...
            <q-card class="kmp-card-rounded q-px-md q-py-sm">
                ...
                <div class="row q-mx-sm q-mb-md q-mt-md">
                    ...
                    <div class="col-12 col-md-4 row justify-end">
                        <q-btn outline color="primary" no-caps label="Tambah Metode" @click="openModal('add')" />
                    </div>
                </div>
                <div class="row q-mx-sm q-mb-md q-mt-md">
                    <div class="col-12">
    ...
    <fixed-modal
        :show="modal_add"
        :title="`${modal_title} Metode`"
        @on-close="modal_add = false"
        :width="$q.platform.is.mobile ? '350': '500'"
    >
      <q-card-section class="q-pt-none q-pb-none">
          <div class="text-neutral">Nama Metode<sup class="text-red">*</sup></div>
        ...
        <q-card :style="$q.platform.is.mobile ? 'min-width: 350px' : 'min-width: 500px'" class="q-py-md q-px-sm">
            <q-card-section>
                <q-icon
                    ...
                    v-close-popup
                />
                <div class="text-h6">Detail Metode</div>
            </q-card-section>
    ...
    <q-dialog ref="dialog"  v-model="modal_konfirmasi">
      <q-card>
        ...
        <q-card-section>
          <div class="q-my-md">Apakah Anda yakin akan menghapus metode?</div>
          <div />
        </q-card-section>
...
import {
  ...
  trainingMethodDelete,
  trainingMethodRestore
} from '../../lib/api';
...
export default {
  ...
  methods: {
    getTrainingMethod() {
      trainingMethodList().then(response => {
    ...
    async openModal(selected, method = null) {
      this.$v.$reset();
      if (selected === 'add') {
        ...
        trainingMethodRestore(method.id).then(response => {
          this.$q.notify({
            message: 'Metode telah dipulihkan',
            color: 'blue'
          });
        ...
        this.modal_add = true;
        trainingMethodDetail(method.id).then(response => {
          this.method_id = method.id;
          this.name = method.name;
    ...
    handleDelete() {
      trainingMethodDelete(this.method_id).then(response => {
        this.$q.notify({
          message: 'Metode telah dihapus',
          color: 'blue'
        });
        this.getTrainingMethod();
      });
    ...
    save(selected, method = null) {
      ...
      if (this.$v.$invalid) {
        ...
        if (this.methodPost === 'create') {
          trainingMethodCreate(data).then(response => {
            this.$q.notify({
              message: 'Metode telah ditambahkan',
              color: 'blue'
            });
            this.getTrainingMethod();
          });
        } else if (this.methodPost === 'update') {
          trainingMethodUpdate(this.method_id, data).then(response => {
            this.$q.notify({
              message: 'Metode telah diperbaharui',
              color: 'blue'
            });
            this.getTrainingMethod();
          });
