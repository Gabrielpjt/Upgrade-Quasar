      ...
      <div>
        ...
        <div v-if="item.post_type === 'Article'">
          <q-card-section>
            ...
            <div class="kmp-post-text-title q-mt-md cursor-pointer"  @click="openUrl(item.id,item.post_channel.channel_id)">
              {{ item.title }}
            </div>
...
export default {
  props: ['is_data', 'today', 'hours'],
  components: { Avatar, PostLoading, PostStatus, KnowledgeListFile, LinkPrevue, PostListMedia, FixedModal, EventRsvp, EventExpertise },
  data () {
    return {
      leaderboard: {
        title: 'Member',
